...
resource "aws_launch_template" "asg-lt" {
  # After app-listener, create asg-lt! [Number-4]
  name = "phonebook-lt"
...
resource "aws_alb_target_group" "app-lb-tg" {
  # After app-lb, create app_lb-tg! [Number-2]
...
resource "aws_alb" "app-lb" {
  # After alb sg., first create app_lb! [Number-1]
...
resource "aws_alb_listener" "app-listener" {
  # After app-lb-tg, create app-listener! [Number-3]
...
resource "aws_autoscaling_group" "app-asg" {
  # After asg-lt, create app-asg! [Number-5]
  max_size = 3
  min_size = 1
  desired_capacity = 1
  name = "phonebook-asg"
...
resource "aws_route53_record" "phonebook" {
  ...
  name    = "iphonebook.${var.hosted-zone}"
  type    = "A"
  alias {
